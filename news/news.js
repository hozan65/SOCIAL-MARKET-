...
function setActiveTab(cat){
    document.querySelectorAll(".news-tab").forEach((b)=> b.classList.toggle("active", b.dataset.cat === cat));
}
...
function renderEmpty(grid){ grid.innerHTML = `<div class="news-empty">Bu kategoride haber yok.</div>`; }

function renderGridChunked(grid, items){
    const token = ++lastRenderToken;
    if(!items || items.length===0){ renderEmpty(grid); return; }
...
async function fetchNews({limit=80}={}){
    const r = await fetch(`${API_BASE}/api/news?limit=${encodeURIComponent(limit)}`, { cache:"no-store" });
    const out = await r.json().catch(()=> ({}));
    if(!r.ok) throw new Error(out?.error || `news fetch failed (${r.status})`);
    const list = out?.items || out?.list || out?.data || out;
