...
const orderPair = (a, b) => (String(a) < String(b) ? [String(a), String(b)] : [String(b), String(a)]);
...
export const handler = async (event) => {
    try {
        ...
        // peer profile (name/avatar göstermek için)
        const { data: peer, error: pe } = await sb
            .from("profiles")
        ...
        if (!peer?.appwrite_user_id) return json(404, { error: "User not found" });

        const [user1_id, user2_id] = orderPair(me, to);
        ...
        // create (unique index varsa race condition'da hata alabilir)
